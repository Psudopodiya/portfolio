      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 w-full">
        {projects.map((project, index) => (
          <ScrollReveal key={index} delay={index * 0.15}>
            <motion.div
              whileHover={{ y: -8 }}
              className={`relative w-full aspect-[3/4] border shadow-sm rounded-2xl overflow-hidden group transition-all duration-500 ease-in-out backdrop-blur-md bg-background/20
              ...
              <div className="absolute bottom-0 left-0 w-full h-full flex flex-col items-center justify-between gap-4 md:gap-6 p-4 md:p-6 transition-all">
                ...
                <div className="flex flex-wrap gap-1 justify-center items-center">
                  {project.tech.map((tech, i) => (
                    <span
                      key={i}
                      className="rounded-xl border px-2 py-1 text-xs md:text-sm backdrop-blur-sm bg-background/30"
                    >
                      {tech}
                    </span>
                  ))}
                ...
                <div className="inline-flex flex-wrap items-center gap-3 md:gap-5 justify-center relative">
                  <motion.div
                    whileHover={{ y: -2 }}
                    whileTap={{ scale: 0.95 }}
                    className={`border px-2 py-1 md:px-3 md:py-1 rounded-2xl relative transition-all hover:shadow-sm ease-in-out duration-300 
