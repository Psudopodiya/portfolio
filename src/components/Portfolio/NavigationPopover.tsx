...
function NavigationPopover({ theme }: NavigationPopoverProps) {
  ...
  return (
    <div className="fixed sm:bottom-5 bottom-4 left-4 z-50">
      ...
      >
        <motion.button
          className={`flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full shadow-lg border ${theme.border_base_color} ${theme.text_base} ${theme.background_secondary}`}
              ...
              <nav className="flex flex-col space-y-1">
                {sections.map((section) => (
                  <Link
                    ...
                    smooth={true}
                    duration={500}
                    className={`px-4 py-2 text-sm rounded transition-colors cursor-pointer ${theme.hover_background_base}`}
                  >
                    {section.name}
                  </Link>
                ))}
